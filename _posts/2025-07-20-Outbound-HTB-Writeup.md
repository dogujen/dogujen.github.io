...
def decrypt_password(encrypted_password, key="rcmail-!24ByteDESkey*Str"):</br>
    try:</br>  
        des_key = key.encode('utf-8')  </br>
        data = b64decode(encrypted_password)</br>  
        iv = data[:8]  </br>
        ciphertext = data[8:]</br>
        cipher = DES3.new(des_key, DES3.MODE_CBC, iv=iv)  </br>
        decrypted = cipher.decrypt(ciphertext)</br>
        return decrypted.rstrip(b"\0").decode('utf-8', errors='ignore')</br>
    except Exception as e:  </br>
        return f"Error: {str(e)}"</br>
encrypted = "L7Rv00A8TuwJAr67kITxxcSgnIk25Am/"  </br>
print(f"Decrypted password: {decrypt_password(encrypted)}")</br>
</code>
